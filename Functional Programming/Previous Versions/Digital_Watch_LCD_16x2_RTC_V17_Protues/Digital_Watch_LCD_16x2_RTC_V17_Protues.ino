...
void ShowTimeOnMainScreen() {
  if ((!IsShowAlarmTime)) {
    ...
    if (HoursType == _24Hours) {
      ...
      if (PushButton4_Value == LOW || digitalRead(PushButton5) == LOW)
...
int ChangeValueForIsAllowCountStopWatch = 0; 

void InterruptOperationsForPushButton4InStopWatch()
{
  GetValueOfPushButton4();
  if(CurrentScreen==1 &&  ChangeValueForIsAllowCountStopWatch != 1)
  {
    IsAllowCountStopWatch ^= 1;
...
void setup() {
 ...
 // Serial.begin(9600);
  ...
  attachInterrupt(digitalPinToInterrupt(InterruptPin), InterruptOperationsForPushButton4InStopWatch, CHANGE );
