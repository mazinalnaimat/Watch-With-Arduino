...
//byte PushButton4 = 5 ;  //the right Up PushButton
#define PushButton4 5  //the middle PushButton
...
void IncreaseMilliSeconds(int& _CountMilliSeconds, int _MaxCountMilliSeconds) {
  ...
  if (_CountMilliSeconds == _MaxCountMilliSeconds) {
    ...
    if(now.Second() <59)
     {
      ...
      RtcDateTime(now.Year(), now.Month(), now.Day(), now.Hour(), now.Minute(),  now.Second() +1);
    ...
    {
            RtcDateTime(now.Year(), now.Month(), now.Day(),now.Hour(), now.Minute(),  0);
    }
  }
...
void UpdateStopTime(sTime _Time, sTime& _StopWatchTime, int& _PreviousMilliSecond, bool _CountPushButton4ForStopWatch) {
