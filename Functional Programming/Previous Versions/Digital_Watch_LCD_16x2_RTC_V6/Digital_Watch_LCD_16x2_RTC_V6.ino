...
bool IsAllowAlarm(bool InputPull_Type = Input_PullUp) {
  // Previous_PushButton4_Value = PushButton4_Value;
  // Previous_PushButton3_Value = PushButton3_Value;
...
void UpdateAlarmNapTimeOperations() {
  _AllowAlarm = false;
  _AllowAlarmNap = true;
  CountForNapTimes++;
...
void ShowTimeAndDateScreen() {
  ...
  if ((PushButton2_Value == LOW && PushButton3_Value == LOW && Input_PullType == Input_PullDown) || (PushButton2_Value == HIGH && PushButton3_Value == HIGH && Input_PullType == Input_PullUp)) {
    if (HoursType == _24Hours)
...
void setup() {
  ...
  */
  ...
  //Rtc.SetDateTime(RtcDateTime(now.Year(), now.Month(), 17, now.Hour(), now.Minute(), now.Second()));


  UpdateHijriDateAndDayNameFrom_EEPROM();
  Timer1.initialize(10000);
  Timer1.attachInterrupt(IncreaseCountAlarmPeep);
